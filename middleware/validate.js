...
export const validateSignup = async (req, res, next) => {
   ...
   try {
      const decoded = jwt.verify(verificationToken, process.env.SECRET_KEY_VERIFY);

      const maybeUser = await getUserByEmail(decoded.email);
      if (maybeUser) {
         return res.status(400).send("Neej, vad synd, du får inte regristrera dig");
      }
      const maybeInvited = await db.invitedUser.findOne({ where: { email: decoded.email } });
      if (!maybeInvited) {
